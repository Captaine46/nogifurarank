...
完整操作、TOP75 完整性檢查、續抓方式、離線重建 HTML 與 PNG 的命令請參閱：
...
既有 `summary.json` 可直接產生四張 TOP50 圖、四張前月比圖與兩張新登場注目卡比較圖，
不會重新訪問 API：
...
若輸出目錄的上一個月份有完整快照，程式會自動選擇該月份日期最新的
`summary.json`，另外建立「お見立て会 TOP20／個人ランキング TOP75 ×
防衛／攻撃」四張採用数・採用率前月比圖。每張圖左側為上昇 TOP25、右側為
低下 TOP25，顯示前月與今月的採用率／採用編成数，以及實際増減編成数與
採用率差。`新規` 表示前月沒有採用實績。
