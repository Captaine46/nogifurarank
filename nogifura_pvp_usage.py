...
<main class="shell">
  <section class="hero">
    ...
    <div class="stamp"><div>データ作成日時（日本時間）</div><strong id="generated-at">—</strong><div id="coverage-text"></div></div>
  </section>
...
const jstDateTime=new Intl.DateTimeFormat('ja-JP',{timeZone:'Asia/Tokyo',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false});
...
function init(){const generatedAt=INDEX.generatedAt?new Date(INDEX.generatedAt):null;$('generated-at').textContent=generatedAt&&!Number.isNaN(generatedAt.getTime())?jstDateTime.format(generatedAt):'—';const c=INDEX.coverage||{};const pc=INDEX.personalRankingCoverage||{};const top=pc.availableTop||0;$('personal-title').textContent=`個人ランキング TOP${top}`;$('coverage-text').textContent=`お見立て会 ${c.rankingChannels||0}/${c.requestedChannels||0} CH · 個人ランキング ${pc.rankingChannels||0}/${pc.requestedChannels||0} CH`;const missing=(INDEX.missing||[]).length;if(missing){const warning=$('warning');warning.style.display='block';warning.textContent=`未取得データが ${missing} 件あります。同じスナップショット名で再実行すると続きから取得できます。`}rebuildChannels();$('channel-select').addEventListener('change',e=>{state.channel=e.target.value;render()});$('rarity-select').addEventListener('change',e=>{state.rarity=e.target.value;renderCards(currentStats())});$('search').addEventListener('input',e=>{state.search=e.target.value.trim().toLocaleLowerCase();render()});document.querySelectorAll('.dataset-button').forEach(button=>button.addEventListener('click',()=>{state.dataset=button.dataset.dataset;document.querySelectorAll('.dataset-button').forEach(x=>x.classList.toggle('active',x===button));rebuildChannels();render()}));document.querySelectorAll('.mode-button').forEach(button=>button.addEventListener('click',()=>{state.mode=button.dataset.mode;document.querySelectorAll('.mode-button').forEach(x=>x.classList.toggle('active',x===button));render()}));document.querySelectorAll('.tab').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('.tab').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.panel').forEach(x=>x.classList.remove('active'));button.classList.add('active');$(`panel-${button.dataset.panel}`).classList.add('active')}));render()}
init();
</script>
</body>
</html>
